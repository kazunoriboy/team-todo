...
type UserQuery struct {
	...
	withLastOrganization        *OrganizationQuery
	withLastProject             *ProjectQuery
...
func (uq *UserQuery) QuerySentInvites() *InviteQuery {
...
// QueryLastOrganization chains the current query on the "last_organization" edge.
func (uq *UserQuery) QueryLastOrganization() *OrganizationQuery {
	query := (&OrganizationClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
...
// QueryLastProject chains the current query on the "last_project" edge.
func (uq *UserQuery) QueryLastProject() *ProjectQuery {
	query := (&ProjectClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (uq *UserQuery) QueryOrganizationMemberships() *OrganizationMemberQuery {
	query := (&OrganizationMemberClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) QueryProjectMemberships() *ProjectMemberQuery {
	query := (&ProjectMemberClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withSentInvites:             uq.withSentInvites.Clone(),
		withLastOrganization:        uq.withLastOrganization.Clone(),
		withLastProject:             uq.withLastProject.Clone(),
		withOrganizationMemberships: uq.withOrganizationMemberships.Clone(),
		withProjectMemberships:      uq.withProjectMemberships.Clone(),
...
func (uq *UserQuery) WithSentInvites(opts ...func(*InviteQuery)) *UserQuery {
	query := (&InviteClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withSentInvites = query
	return uq
}

// WithLastOrganization tells the query-builder to eager-load the nodes that are connected to
// the "last_organization" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithLastOrganization(opts ...func(*OrganizationQuery)) *UserQuery {
	query := (&OrganizationClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withLastOrganization = query
	return uq
}

// WithLastProject tells the query-builder to eager-load the nodes that are connected to
// the "last_project" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithLastProject(opts ...func(*ProjectQuery)) *UserQuery {
	query := (&ProjectClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withLastProject = query
	return uq
}
...
func (uq *UserQuery) WithOrganizationMemberships(opts ...func(*OrganizationMemberQuery)) *UserQuery {
	query := (&OrganizationMemberClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withOrganizationMemberships = query
	return uq
}
...
func (uq *UserQuery) WithProjectMemberships(opts ...func(*ProjectMemberQuery)) *UserQuery {
	query := (&ProjectMemberClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withProjectMemberships = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	var (
		...
		loadedTypes = [7]bool{
			...
			uq.withProjects != nil,
			uq.withSentInvites != nil,
			uq.withLastOrganization != nil,
			uq.withLastProject != nil,
			uq.withOrganizationMemberships != nil,
			uq.withProjectMemberships != nil,
		}
	)
	...
	if query := uq.withSentInvites; query != nil {
		if err := uq.loadSentInvites(ctx, query, nodes,
			func(n *User) { n.Edges.SentInvites = []*Invite{} },
			func(n *User, e *Invite) { n.Edges.SentInvites = append(n.Edges.SentInvites, e) }); err != nil {
			return nil, err
		}
	}
	if query := uq.withLastOrganization; query != nil {
		if err := uq.loadLastOrganization(ctx, query, nodes, nil,
			func(n *User, e *Organization) { n.Edges.LastOrganization = e }); err != nil {
			return nil, err
		}
	}
	if query := uq.withLastProject; query != nil {
		if err := uq.loadLastProject(ctx, query, nodes, nil,
			func(n *User, e *Project) { n.Edges.LastProject = e }); err != nil {
			return nil, err
		}
	}
	if query := uq.withOrganizationMemberships; query != nil {
		if err := uq.loadOrganizationMemberships(ctx, query, nodes,
	...
	if query := uq.withProjectMemberships; query != nil {
		if err := uq.loadProjectMemberships(ctx, query, nodes,
...
func (uq *UserQuery) loadSentInvites(ctx context.Context, query *InviteQuery, nodes []*User, init func(*User), assign func(*User, *Invite)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(invite.FieldInvitedByID)
	}
	query.Where(predicate.Invite(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.SentInvitesColumn), fks...))
	}))
...
func (uq *UserQuery) loadLastOrganization(ctx context.Context, query *OrganizationQuery, nodes []*User, init func(*User), assign func(*User, *Organization)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*User)
	for i := range nodes {
		if nodes[i].LastOrgID == nil {
			continue
		}
		fk := *nodes[i].LastOrgID
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "last_org_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (uq *UserQuery) loadLastProject(ctx context.Context, query *ProjectQuery, nodes []*User, init func(*User), assign func(*User, *Project)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*User)
	for i := range nodes {
		if nodes[i].LastProjectID == nil {
			continue
		}
		fk := *nodes[i].LastProjectID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "last_project_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (uq *UserQuery) loadOrganizationMemberships(ctx context.Context, query *OrganizationMemberQuery, nodes []*User, init func(*User), assign func(*User, *OrganizationMember)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
...
func (uq *UserQuery) loadProjectMemberships(ctx context.Context, query *ProjectMemberQuery, nodes []*User, init func(*User), assign func(*User, *ProjectMember)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
...
func (uq *UserQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := uq.ctx.Fields; len(fields) > 0 {
		...
		if uq.withLastOrganization != nil {
			_spec.Node.AddColumnOnce(user.FieldLastOrgID)
		}
		if uq.withLastProject != nil {
			_spec.Node.AddColumnOnce(user.FieldLastProjectID)
		}
	}
