...
func (oq *OrganizationQuery) QueryMembers() *UserQuery {
...
// QueryLastAccessedBy chains the current query on the "last_accessed_by" edge.
func (oq *OrganizationQuery) QueryLastAccessedBy() *UserQuery {
	query := (&UserClient{config: oq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (oq *OrganizationQuery) QueryOrganizationMemberships() *OrganizationMemberQuery {
	query := (&OrganizationMemberClient{config: oq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (oq *OrganizationQuery) Clone() *OrganizationQuery {
	...
	return &OrganizationQuery{
		...
		withLastAccessedBy:          oq.withLastAccessedBy.Clone(),
		withOrganizationMemberships: oq.withOrganizationMemberships.Clone(),
...
func (oq *OrganizationQuery) WithMembers(opts ...func(*UserQuery)) *OrganizationQuery {
	query := (&UserClient{config: oq.config}).Query()
...
// WithLastAccessedBy tells the query-builder to eager-load the nodes that are connected to
// the "last_accessed_by" edge. The optional arguments are used to configure the query builder of the edge.
func (oq *OrganizationQuery) WithLastAccessedBy(opts ...func(*UserQuery)) *OrganizationQuery {
	query := (&UserClient{config: oq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	oq.withLastAccessedBy = query
	return oq
}
...
func (oq *OrganizationQuery) WithOrganizationMemberships(opts ...func(*OrganizationMemberQuery)) *OrganizationQuery {
	query := (&OrganizationMemberClient{config: oq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	oq.withOrganizationMemberships = query
	return oq
}
...
func (oq *OrganizationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Organization, error) {
	var (
		...
		loadedTypes = [5]bool{
			...
			oq.withProjects != nil,
			oq.withInvites != nil,
			oq.withLastAccessedBy != nil,
			oq.withOrganizationMemberships != nil,
		}
	)
	...
	if query := oq.withLastAccessedBy; query != nil {
		if err := oq.loadLastAccessedBy(ctx, query, nodes,
			func(n *Organization) { n.Edges.LastAccessedBy = []*User{} },
			func(n *Organization, e *User) { n.Edges.LastAccessedBy = append(n.Edges.LastAccessedBy, e) }); err != nil {
			return nil, err
		}
	}
	if query := oq.withOrganizationMemberships; query != nil {
		if err := oq.loadOrganizationMemberships(ctx, query, nodes,
			func(n *Organization) { n.Edges.OrganizationMemberships = []*OrganizationMember{} },
			func(n *Organization, e *OrganizationMember) {
...
func (oq *OrganizationQuery) loadMembers(ctx context.Context, query *UserQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *User)) error {
	...
	query.Where(func(s *sql.Selector) {
		...
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(organization.MembersPrimaryKey[0]))
...
func (oq *OrganizationQuery) loadLastAccessedBy(ctx context.Context, query *UserQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *User)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Organization)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(user.FieldLastOrgID)
	}
	query.Where(predicate.User(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(organization.LastAccessedByColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.LastOrgID
		if fk == nil {
			return fmt.Errorf(`foreign-key "last_org_id" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "last_org_id" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (oq *OrganizationQuery) loadOrganizationMemberships(ctx context.Context, query *OrganizationMemberQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *OrganizationMember)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Organization)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.OrganizationMember(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(organization.OrganizationMembershipsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
