...
func (pq *ProjectQuery) QueryInvites() *InviteQuery {
...
func (pq *ProjectQuery) QueryLastAccessedBy() *UserQuery {
	query := (&UserClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (pq *ProjectQuery) QueryProjectMemberships() *ProjectMemberQuery {
...
func (pq *ProjectQuery) Clone() *ProjectQuery {
	...
	return &ProjectQuery{
		...
		withLastAccessedBy:     pq.withLastAccessedBy.Clone(),
		withProjectMemberships: pq.withProjectMemberships.Clone(),
...
func (pq *ProjectQuery) WithInvites(opts ...func(*InviteQuery)) *ProjectQuery {
	query := (&InviteClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *ProjectQuery) WithLastAccessedBy(opts ...func(*UserQuery)) *ProjectQuery {
	query := (&UserClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withLastAccessedBy = query
	return pq
}
...
func (pq *ProjectQuery) WithProjectMemberships(opts ...func(*ProjectMemberQuery)) *ProjectQuery {
	query := (&ProjectMemberClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withProjectMemberships = query
	return pq
}
...
func (pq *ProjectQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Project, error) {
	var (
		...
		loadedTypes = [5]bool{
			...
			pq.withMembers != nil,
			pq.withInvites != nil,
			pq.withLastAccessedBy != nil,
			pq.withProjectMemberships != nil,
		}
	)
	...
	if query := pq.withLastAccessedBy; query != nil {
		if err := pq.loadLastAccessedBy(ctx, query, nodes,
			func(n *Project) { n.Edges.LastAccessedBy = []*User{} },
			func(n *Project, e *User) { n.Edges.LastAccessedBy = append(n.Edges.LastAccessedBy, e) }); err != nil {
			return nil, err
		}
	}
	if query := pq.withProjectMemberships; query != nil {
		if err := pq.loadProjectMemberships(ctx, query, nodes,
			func(n *Project) { n.Edges.ProjectMemberships = []*ProjectMember{} },
			func(n *Project, e *ProjectMember) { n.Edges.ProjectMemberships = append(n.Edges.ProjectMemberships, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *ProjectQuery) loadMembers(ctx context.Context, query *UserQuery, nodes []*Project, init func(*Project), assign func(*Project, *User)) error {
	...
	query.Where(func(s *sql.Selector) {
		...
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(project.MembersPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(project.MembersPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(project.MembersPrimaryKey[1]))
...
func (pq *ProjectQuery) loadInvites(ctx context.Context, query *InviteQuery, nodes []*Project, init func(*Project), assign func(*Project, *Invite)) error {
...
func (pq *ProjectQuery) loadLastAccessedBy(ctx context.Context, query *UserQuery, nodes []*Project, init func(*Project), assign func(*Project, *User)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Project)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(user.FieldLastProjectID)
	}
	query.Where(predicate.User(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(project.LastAccessedByColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.LastProjectID
		if fk == nil {
			return fmt.Errorf(`foreign-key "last_project_id" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "last_project_id" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (pq *ProjectQuery) loadProjectMemberships(ctx context.Context, query *ProjectMemberQuery, nodes []*Project, init func(*Project), assign func(*Project, *ProjectMember)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Project)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.ProjectMember(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(project.ProjectMembershipsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
